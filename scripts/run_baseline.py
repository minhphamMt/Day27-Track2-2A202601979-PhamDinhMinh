...
def main() -> None:
    ...
    # Use the recent global baseline for the healthy synthetic feed.  The
    # detector itself supports same-weekday context; callers can opt into it
    # when the producer publishes a genuinely seasonal metric.
    ...
    # Demo SLO: one data-quality check event for this run.  Any failed order or
    # KB contract is a bad event; severity still controls the response action.
    bad = 1 if (failed or kb_failed) else 0

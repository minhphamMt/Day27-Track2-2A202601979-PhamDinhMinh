...
- `gx/validate_orders.py`: Suite/ValidationDefinition/Checkpoint với metadata action.
- `dbt_project/`: generic + singular data tests và dbt unit test cho join/SCD.
- `observability/anomaly.py`: robust baseline, seasonality, MAD và zero-MAD handling.
- `observability/distribution.py`: mean, spread và quantile drift.
- `observability/slo.py`: multi-window burn-rate policy.
- `observability/lineage.py`: dataset và column transitive lineage.
- `observability/rag_metrics.py`: text-length và embedding-norm drift.
- `reports/incident_report.md`: incident report có evidence và action items.
